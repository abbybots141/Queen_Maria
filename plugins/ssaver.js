  ...
  async (message) => {
    try {
      ...
      if (mm) {
        ...
        message.send("*Reply to a WhatsApp status to save it*");
      }
    } catch (e) {
...
smd({ on: "quoted" }, async (message, text) => {
  try {
    let mm = message.reply_message && message.reply_message.status
      ? message.reply_message
      : false;
  ...
  async (m) => {
    try {
      await m.send(
        ...
        {
          caption:
            "*Support QUEEN MARIA*\n\nOPay Bank: OPay Digital Services Limited (OPay)\nOpay Account: 9030576943\nName: Nigeria Show Love",
        },
        "img",
        m
      );
